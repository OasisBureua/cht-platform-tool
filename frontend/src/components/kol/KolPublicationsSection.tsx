    ...
    <article className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
      <h2 className="flex items-center gap-2 text-sm font-bold text-zinc-900 dark:text-zinc-100">
        ...
        {data.total > data.items.length ? (
          <span className="text-xs font-normal text-zinc-500">({data.total} indexed)</span>
        ) : null}
      </h2>
      <ul className="mt-3 space-y-3">
        {data.items.map((pub, idx) => {
          ...
          return (
            <li key={`${pub.title}-${idx}`} className="border-b border-zinc-100 pb-3 last:border-0 last:pb-0 dark:border-zinc-800">
              {pub.url ? (
                <a
                  href={pub.url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="group inline-flex items-start gap-1.5 text-[15px] font-semibold leading-snug text-zinc-900 hover:text-brand-700 dark:text-zinc-100 dark:hover:text-brand-400"
                >
                ...
                <p className="text-[15px] font-semibold leading-snug text-zinc-900 dark:text-zinc-100">{pub.title}</p>
              )}
              {meta ? <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">{meta}</p> : null}
            </li>
          );
        })}
      </ul>
