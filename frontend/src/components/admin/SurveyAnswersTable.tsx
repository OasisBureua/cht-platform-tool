...
export function SurveyAnswersTable(props: {
  ...
  return (
    <table
      className={[
        'w-full border border-gray-100 rounded-[6px] overflow-hidden',
      ...
      <tbody>
        {rows.map((row) => (
          <tr key={row.label} className="border-b border-gray-50 last:border-0">
            <td className="py-1.5 px-2 font-medium text-muted-foreground align-top w-2/5">{row.label}</td>
            <td className="py-1.5 px-2 text-muted-foreground">{row.value}</td>
          </tr>
        ))}
      </tbody>
    </table>
