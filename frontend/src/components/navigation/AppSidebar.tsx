  ...
  return (
    <aside className="sticky top-0 hidden h-screen w-[112px] shrink-0 self-start flex-col bg-white/70 shadow-[6px_0_36px_-20px_rgba(0,0,0,0.09)] backdrop-blur-xl backdrop-saturate-150 md:flex dark:bg-zinc-950/85 dark:shadow-[8px_0_40px_-22px_rgba(0,0,0,0.55)]">
      <div className="flex h-[88px] w-full shrink-0 items-center justify-center border-b border-zinc-200/25 dark:border-zinc-800/60">
        <Link
          to="/app/home"
          className="flex h-[clamp(56px,8.5vh,76px)] w-[88px] items-center justify-center rounded-2xl text-accent-600 transition-[color,opacity,transform] duration-200 ease-out hover:text-accent-700 hover:opacity-95 active:scale-[0.96] dark:text-accent-400 dark:hover:text-accent-300"
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              [
                ...
                isActive
                  ? 'bg-accent-100 text-accent-950 shadow-[inset_0_0_0_1px_rgba(49,105,149,0.18),0_8px_24px_-12px_rgba(49,105,149,0.22)] ring-2 ring-accent-500/25 ring-offset-0 dark:bg-accent-600 dark:text-white dark:ring-accent-400/35 dark:shadow-[0_8px_28px_-12px_rgba(37,99,235,0.35)]'
                  : 'text-zinc-900 hover:bg-accent-50/90 hover:text-zinc-950 dark:text-zinc-100 dark:hover:bg-zinc-900/90 dark:hover:text-white',
              ].join(' ')
            }
          >
            {({ isActive }) => (
