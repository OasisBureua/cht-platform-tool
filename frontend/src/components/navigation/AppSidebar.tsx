...
export default function AppSidebar() {
  return (
    <aside className="sticky top-0 hidden h-screen w-[112px] shrink-0 self-start flex-col bg-white/70 shadow-[6px_0_36px_-20px_rgba(0,0,0,0.09)] backdrop-blur-xl backdrop-saturate-150 md:flex /85 dark:shadow-[8px_0_40px_-22px_rgba(0,0,0,0.55)]">
      ...
      <nav
        className="app-sidebar-nav flex flex-1 flex-col items-center justify-start gap-2 overflow-y-auto overflow-x-hidden px-1.5 py-3"
        aria-label="Primary"
      >
        {APP_NAV_ITEMS.map(({ to, label, icon: Icon, iconTone, end }) => (
          <NavLink
            ...
            className={({ isActive }) =>
              [
                'flex h-16 w-[88px] shrink-0 flex-col items-center justify-center gap-1.5 rounded-card px-1.5 py-2 text-center transition-[color,background-color,transform,box-shadow] duration-200 ease-[cubic-bezier(0.2,0,0,1)] active:scale-[0.96]',
                isActive
