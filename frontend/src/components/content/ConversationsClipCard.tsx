...
export function ConversationsClipCard({ item, href }: ConversationsClipCardProps) {
  ...
  return (
    <Link
      to={href}
      state={{ clip: item }}
      className="group flex min-h-0 min-w-0 flex-col overflow-hidden rounded-[6px] bg-card shadow-[0_1px_0_rgba(0,0,0,0.04),0_6px_20px_-10px_rgba(0,0,0,0.1)] transition-[transform,box-shadow] duration-200 ease-[cubic-bezier(0.2,0,0,1)] hover:shadow-[0_1px_0_rgba(0,0,0,0.05),0_10px_28px_-10px_rgba(0,0,0,0.15)] active:scale-[0.96] dark:bg-zinc-900"
    >
      ...
      <div className="flex min-h-[5.25rem] flex-1 flex-col p-2.5 pt-2">
        <p
          className="line-clamp-2 text-left text-[13px] font-semibold leading-snug text-foreground [overflow-wrap:anywhere] dark:text-zinc-100"
          title={item.title}
        >
          {item.title}
        </p>
