  ...
  return (
    <div className="space-y-8">
      <header className="space-y-3">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="flex items-center gap-2.5 text-foreground">
            <Radio className="h-5 w-5 text-steel-600 dark:text-steel-400" strokeWidth={2} aria-hidden />
            <h1 className="text-balance text-2xl font-bold text-foreground md:text-3xl">LIVE</h1>
          </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-card border border-gray-100/90 bg-card p-12 text-center shadow-[0_1px_0_rgba(0,0,0,0.04),0_8px_28px_-12px_rgba(0,0,0,0.06)]">
          <p className="font-semibold text-foreground">No Live sessions scheduled</p>
          <p className="mt-1 text-sm text-muted-foreground">Check back soon for upcoming sessions.</p>
        </div>
      ) : upcoming.length === 0 && past.length === 0 ? (
        <div className="rounded-card border border-gray-100/90 bg-card p-12 text-center shadow-[0_1px_0_rgba(0,0,0,0.04),0_8px_28px_-12px_rgba(0,0,0,0.06)]">
          <p className="font-semibold text-foreground">No upcoming Live sessions</p>
          <p className="mt-1 text-sm text-muted-foreground">Check back soon for new sessions.</p>
        </div>
      ) : (
        <div className="space-y-8">
          {/* Upcoming */}
          {upcoming.length > 0 && (
            <section className="space-y-3">
              ...
              <div className="divide-y divide-gray-100 overflow-hidden rounded-card border border-gray-100/90 bg-card shadow-[0_1px_0_rgba(0,0,0,0.04),0_8px_28px_-12px_rgba(0,0,0,0.06)]">
                {upcoming.map((w, i) => (
                  <WebinarRow
                    key={w.id}
                    webinar={w}
          ...
          {past.length > 0 && (
            <section className="space-y-3">
              ...
              <div className="divide-y divide-gray-100 overflow-hidden rounded-card border border-gray-100/90 bg-card opacity-70 shadow-[0_1px_0_rgba(0,0,0,0.04),0_8px_28px_-12px_rgba(0,0,0,0.06)]">
                {past.map((w, i) => (
                  <WebinarRow
                    key={w.id}
                    webinar={w}
...
function WebinarRow({
  ...
  return (
    <Link
      to={`/app/live/${w.id}`}
      className="group flex items-center gap-3 px-4 py-3 transition-[background-color,transform] duration-200 ease-[cubic-bezier(0.2,0,0,1)] hover:bg-muted active:scale-[0.995] sm:gap-4 sm:px-5"
    >
      {/* Left thumbnail */}
      <div className="relative h-14 w-20 shrink-0 overflow-hidden rounded-[6px] bg-muted sm:h-16 sm:w-24">
        <img
          src={imageUrl}
          alt=""
      ...
      <div className="flex-1 min-w-0 space-y-1">
        ...
        {w.hostDisplayName && (
          <p className="text-xs text-muted-foreground truncate">
            ...
            {w.hostDisplayName}
            {w.hostBio ? <span className="text-muted-foreground"> · {w.hostBio}</span> : null}
          </p>
        )}
      ...
      <ChevronRight className="h-4 w-4 shrink-0 text-muted-foreground transition-[color,transform] duration-200 ease-[cubic-bezier(0.2,0,0,1)] group-hover:translate-x-0.5 group-hover:text-muted-foreground" />
    </Link>
  );
}
