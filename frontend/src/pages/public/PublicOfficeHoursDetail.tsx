...
import { ChevronLeft, Calendar, Clock, LogIn, UserPlus, Video } from 'lucide-react';
...
export default function PublicOfficeHoursDetail() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { user, isAuthenticated } = useAuth();

  const { data: session, isLoading, isError } = useQuery({
    queryKey: ['office-hours', 'public', id],
    queryFn: () => webinarsApi.getOfficeHoursById(id!),
    enabled: !!id,
    retry: false,
  });

  const isProgram = session?.source === 'program' && session?.id && !session.id.startsWith('zoom-');
          ...
          <div className="p-6 md:p-8 space-y-6">
            ...
            {showLoginPrompt ? (
              <div className="pt-4 border-t border-border">
                <p className="text-sm font-semibold text-foreground mb-3">Sign in to join this session</p>
