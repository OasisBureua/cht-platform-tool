...
export default function AdminHcpExplorer() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-foreground">HCP Profile Explorer</h1>
      ...
      <div className="bg-card rounded-card border border-border p-5 space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold text-foreground">Profile Lookup</h2>
            {!isLoading && (
          ...
          <button
            ...
            className="rounded-xl border border-border px-4 py-2 text-sm font-semibold text-foreground inline-flex items-center gap-2 hover:bg-muted disabled:opacity-40 transition-colors"
          >
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full rounded-xl border border-border pl-11 pr-4 py-2.5 text-sm focus:border-gray-900 focus:outline-none focus:ring-1 focus:ring-gray-900"
            />
          </div>
          <select
            value={roleFilter}
            onChange={(e) => setRoleFilter(e.target.value)}
            className="rounded-xl border border-border px-4 py-2.5 text-sm font-medium text-foreground sm:w-44 focus:border-gray-900 focus:outline-none focus:ring-1 focus:ring-gray-900"
          >
            {ROLES.map((r) => (
...
function UserRow({ user, compactState }: { user: AdminUser; compactState?: boolean }) {
  ...
  return (
    <div className="bg-card rounded-card border border-border overflow-hidden">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-sm text-muted-foreground truncate">
            ...
            {!compactState && user.state?.trim() ? (
              <span className="text-muted-foreground"> · {user.state.trim()}</span>
            ) : null}
          </p>
        </div>
      ...
      {open ? (
        <div className="border-t border-gray-100 bg-muted px-5 py-4">
          <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-2">Successful payments</p>
          {isFetching ? (
            <Loader2 className="h-5 w-5 animate-spin text-muted-foreground" />
          ) : payments.length === 0 ? (
            <p className="text-sm text-muted-foreground">No paid payouts recorded for this user.</p>
          ) : (
            <ul className="space-y-2">
              {payments.map((p) => (
                <li
                  key={p.id}
                  className="flex flex-wrap items-baseline justify-between gap-2 rounded-lg border border-border bg-card px-3 py-2 text-xs text-foreground"
                >
                  <span className="font-semibold text-foreground">{formatPaidUsd(p.amount)}</span>
                  <span className="text-muted-foreground">{p.type.replace(/_/g, ' ')}</span>
                  <span className="text-muted-foreground w-full sm:w-auto">
                    {p.program?.title ?? '-'}
                    {p.paidAt ? ` · Paid ${new Date(p.paidAt).toLocaleDateString()}` : ''}
                  </span>
                  {p.description?.trim() ? (
                    <span className="text-muted-foreground w-full">{p.description.trim()}</span>
                  ) : null}
                </li>
              ))}
            </ul>
