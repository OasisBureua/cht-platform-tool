...
function getPercentChangeLabel(current: number, previous: number): { label: string; colorClass: string } {
  if (previous === 0) {
    return { label: current > 0 ? '+100%' : '0%', colorClass: current > 0 ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700' };
  }
