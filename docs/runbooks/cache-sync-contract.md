...
- **1h TTL**: safety net for populated keys (`EX 3600`)
...
| TTL | `EX 3600` (1 hour; override via `REDIS_CACHE_TTL_SECONDS` / `CATALOG_CLIPS_CACHE_TTL_SECONDS`) |
...
mediahub-worker completes successful sync
    ...
    → Next user request = cache miss → fetch upstream → store 1h
```
...
| Scenario | Expected |
| -------- | -------- |
| Cache miss | Hub called; key set with 1h TTL |
