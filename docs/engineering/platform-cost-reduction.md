...
**Owner:** Platform  
**Scope:** **Overall AWS / platform spend** — not chatbot-only. Covers CHT Platform (prod + future dev + DR), MediaHub EC2, future `cht-chat`, Content Hub touchpoints, networking (NAT), databases, LLM, and idle non-prod capacity.  
...
| **P1** | Keep **cht-chat** in a **separate repo**; **Fargate Q&A on existing cluster** + **Lambda KB**; no public ALB; small `cht-chat-db` | Keeps chat add-on small vs OpenSearch/EC2/new cluster | Med |
...
## 2. Repo ownership (chat)

**Recommendation:** `cht-chat` lives in a **separate repository** from:
...
| What lives in `cht-chat` repo | RAG API, KB worker, embeddings pipeline, chat Terraform (or chat module consumed by platform env) |
...
| ---- | ------- | ------------ |
| **A. Retire Hub (preferred if product agrees)** | Move must-keep capabilities into CHT / Content Hub / `cht-chat`; turn off EC2; **do not** stand up full `mediahub-api`×2 + worker + reports + Multi-AZ RDS | **Max savings** |
...
1. **Chat** → `cht-chat` + `cht-chat-db` (separate repo; Service Connect).  
...
| Resource | Off | On |
| -------- | --- | --- |
| **ECS services** (`backend`, `worker`, `cht-chat`) | `desiredCount = 0` | Restore normal desired (1) |
...
| `cht-chat` | `desired = 0` when idle; `1` when testing chat |
| `cht-chat-kb` | Never always-on; invoke on demand |
...
| **Service Connect-only `cht-chat`** | No extra public ALB for chat |
...
Use **Fargate Spot** for `cht-chat-kb` and non-critical workers (interruptible). Keep `cht-chat` request path on regular Fargate.
...
| Cross-AZ chatter | Keep chat + `cht-chat-db` in same AZ placement strategy when Single-AZ DB |
...
| `cht-chat-db` | Separate **small** instance; Single-AZ until chat is critical path |
...
| Chat | `cht-chat` desired 1–2 prod; 0 when lightswitch off in dev |
...
2. Lambda (Python) actions:
   - ECS: `UpdateService` desiredCount for `backend`, `worker`, `cht-chat`  
   - RDS: `StopDBInstance` / `StartDBInstance` for `cht-dev-db` (+ `cht-chat-db` if present)  
...
| -------- | -------------- |
| Tagging | `Project`, `Environment`, `Service` (`cht-platform`, `cht-chat`, `mediahub`) on all Terraform resources |
...
| Doc | Relevance |
| --- | --------- |
| [chmbot-migration-architecture.md](./chmbot-migration-architecture.md) | `cht-chat` architecture; Bedrock; pgvector |
