...
|------|-------------------|--------|----------|
| Staging | `staging` | `staging.testapp.communityhealth.media` | `deploy-staging.yml` |
...
Triggers on push to `staging` or `feature/**` branches (docs-only changes are ignored).

1. Run tests (backend, frontend, Terraform validate)
2. Build and push Docker images to ECR
3. Terraform apply (`us-east-1-staging`)
...
Deploy workflows read secrets from GitHub Environments (`staging`, `platform`). Verify before a deploy:
...
1. Merge feature work to `staging` or a `feature/*` branch → auto-deploy to staging
